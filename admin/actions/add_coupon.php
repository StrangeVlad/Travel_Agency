...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (
    empty($code) || empty($description) || empty($discount_type) ||
  ...
  $stmt = $conn->prepare("INSERT INTO coupons (code, description, discount_type, discount_value, min_purchase, start_date, end_date, max_uses, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
  $stmt->bind_param("sssddssss", $code, $description, $discount_type, $discount_value, $min_purchase, $start_date_formatted, $end_date_formatted, $max_uses, $status);

  if ($stmt->execute()) {
    $response['success'] = true;
    $response['message'] = 'Coupon added successfully';
  } else {
    $response['message'] = 'Error adding coupon: ' . $conn->error;
  }
